...
app.post("/api/magnet", async (req, res) => {
  try {
    ...
    const newMagnet = await pool.query(
      `INSERT INTO magnet (image_path, lat, lng, owner, hunter, comment, city, country, favorite, handmade, area) VALUES ('${postData.image_path}', '${postData.lat}', '${postData.lng}', '${postData.owner}', '${postData.hunter}', '${postData.comment}', '${postData.city}', '${postData.country}', '${postData.favorite}','${postData.handmade}', '${postData.area}')`
    );
    res.json(newMagnet);
  } catch (err) {
